...
echo "### ping para R2 usando IPv4 ###"
docker exec cliente1 /bin/ping -n -c 1 10.10.50.20

echo "### ping para R2 usando IPv6 ###"
docker exec cliente1 /bin/ping6 -n -c 1 2001:db8:2021:50::20
...
docker exec cliente1 /usr/bin/curl -u inf500:inf500 ftp://10.10.100.20/arquivo.txt

echo "### curl para ftp usando IPv6"
docker exec cliente1 /usr/bin/curl -u inf500:inf500 ftp://[2001:db8:2021:100::20]/arquivo.txt

echo "### testando porta 22 no servidor ssh usando IPv4"
docker exec cliente1 timeout 5 bash -c "</dev/tcp/10.10.100.30/22"; echo $?

echo "### testando porta 22 no servidor ssh usando IPv6"
docker exec cliente1 timeout 5 bash -c "</dev/tcp/2001:db8:2021:100::30/22"; echo $?
